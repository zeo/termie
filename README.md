...
Drag a tab out to make a window, or drop it into another window opened with `Ctrl+Shift+N` to combine them again. The tab keeps its live pane tree, shell processes, title, color, and status. Dragging a window's only tab moves that window, and docking the final tab or pane closes the emptied source window.
