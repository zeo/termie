...
The command line takes Windows Terminal's layout verbs too: `termie new-tab -d ~/src ; split-pane -H --shell bash` opens a window with that layout (`nt`/`sp` for short; `-V` splits beside and is the default, `-H` below). Scripted windows never overwrite your saved session. For automation and demos, `--drive script.txt` plays timed input through the normal handlers: `500 key ctrl+shift+m`, `100 type hello`, `50 pointer 320 140`, and `0 mouse down` / `100 mouse up`. Pointer coordinates are client pixels and may sit outside the window to exercise tab or pane tear-out. `pointer-window 1 300 120` aims at client coordinates in another Termie window without hard-coding its desktop position. `assert windows 2`, `assert tabs 3`, and `assert panes 4` check process-wide live counts. `assert window-tabs 1 2` and `assert window-panes 0 3` inspect one window, with the main window at index 0. A failed assertion makes the process return an error after `exit`. Windows creates this window without activation. Linux compositors control activation, so a drive window can receive focus there.
