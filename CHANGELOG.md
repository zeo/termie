...
- **Pointer-driven drag tests use Termie's real handlers.** `termie --drive` accepts client-space `pointer x y` and `mouse down` / `mouse up` steps, including coordinates outside the window. Pane docking, pane-to-tab conversion, tab tear-out, and multi-window shutdown can run without synthesizing desktop input.
