...
- Windows releases now use the signed shared installer payload and no longer build or execute WiX.
...
- **Drag destinations clear when the pointer leaves them.** Passing over another Termie window and then releasing on the desktop no longer sends the tab or pane back to the window you already left. A destination that closes during a tab drag is rejected without losing the live tab.
...
- **Pointer-driven drag tests prove multi-window topology.** `termie --drive` can route a pointer to client coordinates in an indexed Termie window, then assert process-wide or per-window tab and pane counts. A failed or unfinished script exits with an error. Pane docking, pane-to-tab conversion, tab tear-out, window merging, and multi-window shutdown now have deterministic native paths without desktop input synthesis, and the complete drag matrix runs on the Windows CI machine after every build.
- **Pane mode explains itself while it is open.** Entering it outlines every pane, attaches a four-tick grab plate to each top edge, and replaces the normal status readout with responsive mouse and keyboard hints. Dock and resize stay visible at narrow or high-DPI sizes; focus, split, close, zoom, and pop-out appear as room allows. The grab cursor and docking preview take over once the pointer moves.

### Platform
- **Multi-window shutdown is clean on Linux.** Termie releases the Wayland clipboard while its display connection is alive, waits for each GPU to finish before dropping its swapchain, and avoids a Vulkan validation-layer teardown crash on secondary windows unless validation was explicitly requested.
...
- **Linux can make Termie the default terminal.** The desktop entry declares the `xdg-terminal-exec` command and directory arguments, while KDE Plasma also gets its native `kdeglobals` application and service settings. The existing "default terminal" palette action puts Termie first for the active desktop, toggles back to the exact previous choice, and keeps a moved or updated executable path current.
- **Windows stay inside the screen.** Fresh, restored, moved, torn-off, and test-driven windows are capped before their first visible frame and whenever they cross monitors. The cap now includes the outer window frame, so no part of a capture window can exceed the display even with server-side decorations. The normal 1000×640 size stays in logical units on fractional-scale Wayland displays.
...
- **termie runs on Linux.** The same rendering, terminal emulation, and plugin core build and run on X11 and Wayland. Configuration, data, state, and cache follow their separate XDG base directories with migration from the preview layout. Bash, zsh, and fish emit full command lifecycle, exit-status, prompt, and cwd marks. Clipboard uses the Wayland or X11 selection, links open with `xdg-open`, desktop theme changes come through the portal, and the plugin sandbox is a bubblewrap jail. Windows-only controls stay out of the Linux interface. CI builds, tests, and lints Linux pull requests, and tagged releases include an x86-64 archive with desktop integration.
- **Closed the Linux preview audit.** Bubblewrap now clears the host environment before restoring a fixed runtime set. Marketplace ZIPs use `unzip` after path validation and symlink rejection. Ctrl-click links, cross-platform file-drop routing, Wayland size restore, X11 urgency clearing, zsh `ZDOTDIR`, Linux update wording, and the dead Mica control are corrected.
- **Dropped the `wayland-scanner` git patch.** crates.io still ships `wayland-scanner` 0.31.10 on the vulnerable `quick-xml` 0.39 (RUSTSEC-2026-0194/0195), and upstream master's scanner no longer generates 0.31-compatible code, so the old `[patch.crates-io]` pin stopped compiling once the Linux build actually exercised it. The two advisories are now ignored in `.cargo/audit.toml` with a note — the crate is a build-time codegen dep on the Wayland path that only ever parses the protocol XML bundled inside it, never untrusted input — to be lifted when a fixed release lands.
...
### Interface
- **`send` keybindings type for you.** `ctrl+alt+g = send git status\r` in `keybindings.conf` types the text into the focused pane as if you had — Windows Terminal's `sendInput`, for canned commands and snippets. `\r`/`\n` press enter, `\t` tab, `\e` escape, `\\` a backslash; the generated template documents the form.
...
### Terminal fidelity
- **Full-screen apps keep their retained history under the wheel.** An alternate-screen app that has already produced scrollback now scrolls that history locally. Fresh alternate screens still receive arrow-key wheel translation, so pagers keep working without turning a prompt's message history into the scroll target.
- **Killed TUIs cannot poison the next shell prompt.** Command-finished shell marks now restore the primary screen and clear mouse, focus, synchronized-output, application-cursor, and kitty-keyboard modes. A process terminated by the OOM killer or `SIGKILL` no longer leaves Termie feeding pointer coordinates and private key sequences into Bash.
...
- **Scoped kitty deletes.** `a=d` now honors the `d=` target key: by image id, by id range (`d=r` with `x=`/`y=` bounds, which also clears virtual placements), by z layer, by cell / column / row / cell-plus-z (`d=p/x/y/q`, 1-based coordinates), or whatever placement covers the cursor cell, with the uppercase forms also freeing the stored pixels. A bare delete still clears every placement, and a hard reset (`RIS`) now drops the decoded images along with the screen instead of pinning them until the pane closes.
...
### Project
- **Plugins moved to the [`termie-plugins`](https://github.com/zeo/termie-plugins) registry** — each plugin now lives there as source and is built into the catalog the in-app store fetches, and that's where plugins are contributed. No change to the termie binary or to how the store works.
